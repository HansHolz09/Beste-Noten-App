...
object DemoDataGenerator {
    data class DemoInitData(
        ...
        val lessonStudentBySlotByYear: Map<Int, List<JournalLessonStudentBySlot>>,
        val totalDayStudentCount: JournalDayStudentCount,
        val totalLessonStudentCount: JournalLessonStudentCount,
        val totalLessonStudentBySlot: List<JournalLessonStudentBySlot>,
    )
    ...
    fun generateInitialData(): DemoInitData {
        ...
        val lessonStudentBySlotByYear = mutableMapOf<Int, List<JournalLessonStudentBySlot>>()
        years.forEachIndexed { index, year ->
            ...
            lessonStudentBySlotByYear[year.id] = generateLessonStudentBySlot(student, weekPlan, attendanceData)
        }
        ...
        val totalLessonStudentBySlot =
            lessonStudentBySlotByYear.values
                .flatten()
                .groupBy { it.weekday to it.lessonNr }
                .map { (slot, entries) ->
                    JournalLessonStudentBySlot(
                        studentId = student.id,
                        weekday = slot.first,
                        lessonNr = slot.second,
                        count = entries.sumOf { it.count ?: 0 },
                        presentCount = entries.sumOf { it.presentCount ?: 0 },
                        notPresentCount = entries.sumOf { it.notPresentCount ?: 0 },
                        notPresentWithAbsenceCount = entries.sumOf { it.notPresentWithAbsenceCount ?: 0 },
                        notPresentWithoutAbsenceCount = entries.sumOf { it.notPresentWithoutAbsenceCount ?: 0 },
                    )
        ...
        val school =
            School(
                id = 0,
                customer = true,
                name = "Demonstrationsschule",
                email = "schule@beispiel.de",
                type = "Gymnasium",
                street = "Schulstraße",
                streetNr = "1",
                postalCode = "01234",
        ...
        val user =
            User(
                id = 1234,
                username = "${student.forename} (Demo)",
                email = "${student.forename?.lowercase()}.${student.name?.lowercase()}@schule.de",
                role = "student",
                school = school,
                year = years.last(),
        ...
        return DemoInitData(
            ...
            lessonStudentBySlotByYear = lessonStudentBySlotByYear,
            totalDayStudentCount = totalDayStudentCount,
            totalLessonStudentCount = totalLessonStudentCount,
            totalLessonStudentBySlot = totalLessonStudentBySlot,
        )
    }
    ...
    private fun generateLessonStudentBySlot(
        student: Student,
        weekPlan: List<List<Subject>>,
        attendanceData: DemoAttendanceData,
    ): List<JournalLessonStudentBySlot> {
        val totalMissing = attendanceData.notPresentLessonsWithAbsence + attendanceData.notPresentLessonsWithoutAbsence
        return weekPlan.flatMapIndexed { weekday, lessons ->
            lessons.mapIndexed { lessonIndex, _ ->
                val count = (attendanceData.schoolDays / 5 + Random.nextInt(-2, 3)).coerceAtLeast(1)
                val notPresentCount =
                    ((count * totalMissing.toFloat() / attendanceData.lessonCount).toInt() + Random.nextInt(0, 2))
                        .coerceIn(0, count)
                val withAbsence =
                    if (totalMissing == 0) {
                        0
                    } else {
                        (notPresentCount * attendanceData.notPresentLessonsWithAbsence.toFloat() / totalMissing).toInt()
                    }
                JournalLessonStudentBySlot(
                    studentId = student.id,
                    weekday = weekday + 1,
                    lessonNr = (lessonIndex + 1).toString(),
                    count = count,
                    presentCount = count - notPresentCount,
                    notPresentCount = notPresentCount,
                    notPresentWithAbsenceCount = withAbsence,
                    notPresentWithoutAbsenceCount = notPresentCount - withAbsence,
                )
            }
        }
    }
