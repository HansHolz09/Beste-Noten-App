...
object SmartspacerPrefs {
    private fun prefs(ctx: Context) =
        ctx.getSharedPreferences("smartspacer_prefs", Context.MODE_PRIVATE)

    fun setTokenState(ctx: Context, stored: Boolean) {
        prefs(ctx).edit { putBoolean("tokenState", stored) }
    }

    fun getTokenState(ctx: Context): Boolean =
        prefs(ctx).getBoolean("tokenState", false)

    fun setDay(ctx: Context, week: JournalDay) {
    ...
    fun setLastFetchNow(ctx: Context, nowMillis: Long) {
        prefs(ctx).edit { putLong("last_fetch", nowMillis) }
    }

    fun getLastFetchEpochMillis(ctx: Context): Long =
        prefs(ctx).getLong("last_fetch", 0L)

    @SuppressLint("NewApi")
    fun shouldFetch(now: ZonedDateTime, lastFetchMillis: Long): Boolean {
        val hour = now.hour
        ...
        return java.time.Duration.between(last, now).toMinutes() >= 60
    }
}